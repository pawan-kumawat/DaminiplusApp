...
class OtherCourseSubscriptionScreen extends StatefulWidget {
  final String otherCourseId;
  final String otherCourseName;

  const OtherCourseSubscriptionScreen({
    super.key,
  ...
  @override
  State<OtherCourseSubscriptionScreen> createState() => _OtherCourseSubscriptionScreenState();
}

class _OtherCourseSubscriptionScreenState extends State<OtherCourseSubscriptionScreen> {
  ...
  Future<void> _fetchPlans() async {
    ...
    final results = await Future.wait([
      APIService.getApiCaller(
        context: context,
        url: ApiUrls.getOtherCourseSubscriptions(widget.otherCourseId),
        showLoader: false,
      ),
      APIService.getApiCaller(
        context: context,
    ...
    // Already-subscribed check — is otherCourse ke liye pehle se koi active
    ...
    if (results[1] != "Error" && mounted) {
      try {
        ...
        final match = data.firstWhere(
          (e) => (e['otherCourseId']?.toString() ?? '') == widget.otherCourseId,
          orElse: () => null,
        );
  ...
  void _showPaymentSheet() {
    ...
    showModalBottomSheet(
      ...
      builder: (_) => _OtherCoursePaymentSheet(
        plan: plan,
  ...
  Future<void> _confirmPurchase(String paymentId, {String? couponCode}) async {
    ...
    final response = await APIService.postApiCaller(
      context: context,
      url: ApiUrls.purchaseOtherCourseSubscription,
      body: {
        "otherCourseSubscriptionId": _selectedPlanId,
        "paymentId":          paymentId,
    ...
    await AppSharedPreferencesData.saveOtherCourse(otherCourseId: widget.otherCourseId, otherCourseName: widget.otherCourseName);
    ...
    await showDialog(
      ...
      builder: (_) => _OtherCourseSuccessDialog(otherCourseName: widget.otherCourseName),
    );

    if (!mounted) return;
    // Other Courses purchase-first flow: payment happens here, THEN the
    // student picks a language, THEN home — unlike Competitive Exam where
    // language comes first and payment happens later at a locked chapter.
    Navigator.pushReplacement(
      context,
      MaterialPageRoute(
        builder: (_) => OtherCourseLanguageSelectionScreen(
          otherCourseId: widget.otherCourseId,
          otherCourseName: widget.otherCourseName,
        ),
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    if (!_isLoading && _alreadySubscribed) {
      return _OtherCourseAlreadySubscribedView(
        otherCourseName: widget.otherCourseName,
                        ...
                        children: [
                          ...
                          Container(
                            ...
                            child: Column(
                              children: [
                                ...
                                Text(
                                  '${widget.otherCourseName} Content is Locked',
                                ...
                                Text(
                                  'Subscribe to unlock all subjects for ${widget.otherCourseName}',
                                  style: TextStyle(
                          ...
                          if (_plans.isEmpty)
                            Container(
                              ...
                              child: const Center(
                                child: Text(
                                  'No plans available for this otherCourse.',
...
// ── Already Subscribed ───────────────────────────────────────
class _OtherCourseAlreadySubscribedView extends StatelessWidget {
  final String otherCourseName;
  final String expiryLabel;

  const _OtherCourseAlreadySubscribedView({
    required this.otherCourseName,
              ...
              children: [
                ...
                Text(
                  expiryLabel.isNotEmpty
                      ? 'You already have an active subscription for $otherCourseName, valid till $expiryLabel.'
                      : 'You already have an active subscription for $otherCourseName.',
                  textAlign: TextAlign.center,
                  style: const TextStyle(
                    color: AppColors.textSecondary,
                    fontSize: 14,
...
class _OtherCoursePaymentSheet extends StatefulWidget {
  final Map<String, dynamic> plan;
  final Future<void> Function(String paymentId, String? couponCode) onConfirm;

  const _OtherCoursePaymentSheet({required this.plan, required this.onConfirm});

  @override
  State<_OtherCoursePaymentSheet> createState() => _OtherCoursePaymentSheetState();
}

class _OtherCoursePaymentSheetState extends State<_OtherCoursePaymentSheet> {
...
class _OtherCourseSuccessDialog extends StatelessWidget {
  final String otherCourseName;
  const _OtherCourseSuccessDialog({required this.otherCourseName});
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 8),
            Text(
              '$otherCourseName content is now unlocked. Happy learning!',
              textAlign: TextAlign.center,
