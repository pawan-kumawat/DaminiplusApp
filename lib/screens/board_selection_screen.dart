...
class _BoardSelectionScreenState extends State<BoardSelectionScreen> {
        ...
        _boards = data
            .map<Map<String, dynamic>>(
              (e) => <String, dynamic>{
                ...
                'shortName': e['name']?.toString() ?? 'B',
                'imageUrl': e['imageUrl']?.toString(),
              },
            )
            .toList();
        if (_boards.isNotEmpty) _selectedId = _boards.first['_id'] as String;
        _isLoading = false;
      });
    } else {
  ...
  // ── Explicit typed lookup — no firstWhere ──────────────
  Map<String, dynamic> _selectedBoard() {
    for (final b in _boards) {
      if ((b['_id'] as String?) == _selectedId) return b;
    }
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              const Text(
                'Choose your education board to get the right syllabus.',
              ...
              if (_isLoading)
                ...
                Expanded(
                  child: ListView.separated(
                    ...
                    itemBuilder: (_, i) {
                      final b = _boards[i];
                      final sn = (b['shortName'] as String? ?? '');
                      final logo = sn.isNotEmpty
                          ? sn.substring(0, 1).toUpperCase()
                          : 'B';
                      return _buildBoardCard(
                        id: b['_id'] as String,
                        name: b['shortName'] as String,
                ...
                child: SizedBox(
                  ...
                  child: ElevatedButton(
                    onPressed: _selectedId == null
                        ? null
                        : () {
                            final sel = _selectedBoard();
                            final boardName =
                                (sel['shortName'] as String?) ?? '';
                            Navigator.push(
                              context,
                              MaterialPageRoute(
