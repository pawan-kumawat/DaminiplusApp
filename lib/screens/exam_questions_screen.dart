...
class _ExamQuestionsScreenState extends State<ExamQuestionsScreen> {
  ...
  Future<void> _selectOption(int optionIndex) async {
    ...
    final response = await APIService.postApiCaller(
      context: context,
      url: ApiUrls.submitExamAnswer(questionId),
  ...
  // whichever exam language is currently active on this account (there's
  // no per-screen language param here, unlike the board pillar — exam
  // language lives on the profile, see AppSharedPreferencesData). Once
  // recorded, the button for this question+language never comes back.
  Future<void> _openTeacherRecord() async {
    final question = _questions[_currentIndex];
    final questionId = question['_id'] as String;
    if (questionId.isEmpty) return;
    final languageId = await AppSharedPreferencesData.getExamLanguageId();
  ...
  Future<void> _goToNext() async {
    ...
    if (_currentIndex < _questions.length - 1) {
      ...
      if (!_topicReadOnly) {
        final response = await APIService.postApiCaller(
          context: context,
          url: ApiUrls.completeExamTopic(widget.topicId),
          body: {},
          showLoader: true,
        );
              ...
              child: SingleChildScrollView(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    // this question already has a video for the current exam
                    // language, never shown to students at all. ───────────
                    if (_isTeacher) ...[
                      SizedBox(
                        width: double.infinity,
