...
class OtherCourseLanguageSelectionScreen extends StatefulWidget {
  final String otherCourseId;
  final String otherCourseName;

  const OtherCourseLanguageSelectionScreen({
    super.key,
    required this.otherCourseId,
    this.otherCourseName = '',
  });

  @override
  State<OtherCourseLanguageSelectionScreen> createState() =>
      _OtherCourseLanguageSelectionScreenState();
}

class _OtherCourseLanguageSelectionScreenState
    extends State<OtherCourseLanguageSelectionScreen> {
  ...
  Future<void> _fetchLanguages() async {
    ...
    final response = await APIService.getApiCaller(
      context: context,
      url: ApiUrls.getOtherCourseLanguages(widget.otherCourseId),
      showLoader: false,
    );
  ...
  Future<void> _selectOtherCourse() async {
    if (_selectedId == null) return;

    final response = await APIService.putApiCaller(
      context: context,
      url: ApiUrls.selectOtherCourse,
      body: {
    ...
    if (response != "Error" && mounted) {
      ...
      await AppSharedPreferencesData.saveOtherCourse(
        otherCourseId: widget.otherCourseId,
        otherCourseName: widget.otherCourseName,
        otherCourseLanguageId: _selectedId!,
      );
      if (!mounted) return;
      ...
      if (!hasPlans) {
        Navigator.pushAndRemoveUntil(
          context,
          MaterialPageRoute(
            builder: (_) => ComingSoonScreen(courseName: widget.otherCourseName),
          ),
          (route) => false,
        );
      } else {
        // Selection ke baad seedha padhai shuru honi chahiye — Home
        // par nahi bhejte. MainScreen stack ke base par rehta hai
        // (back button se Home aa sake), subjects list uske upar push
        // hoti hai. `navigator` pehle hi capture kar liya kyunki baad
        // me widget/context dispose ho jaata hai. IMPORTANT:
        ...
        final otherCourseId = widget.otherCourseId;
        final otherCourseName = widget.otherCourseName;
        final otherCourseLanguageId = _selectedId!;
        navigator.pushAndRemoveUntil(
        ...
        navigator.push(
          MaterialPageRoute(
            builder: (_) => CourseSubjectsScreen(
              kind: 'otherCourse',
              courseTitle: otherCourseName.isNotEmpty ? otherCourseName : 'Subjects',
              otherCourseId: otherCourseId,
              languageId: otherCourseLanguageId,
            ),
          ),
        );
      }
              ...
              Padding(
                ...
                child: SizedBox(
                  ...
                  child: ElevatedButton(
                    onPressed: _selectedId == null ? null : _selectOtherCourse,
