// ─────────────────────────────────────────────────────────────
// chapters_screen.dart  — UPDATED with AppLocalizations
// ─────────────────────────────────────────────────────────────
...
class _ChaptersScreenState extends State<ChaptersScreen> {
  ...
  Future<void> _fetchData() async {
    ...
    final results = await Future.wait([
      APIService.getApiCaller(
        context: context,
        url: ApiUrls.getChapters(widget.subjectId, widget.languageId),
        showLoader: false,
      ),
      APIService.getApiCaller(
        context: context,
        url: ApiUrls.getTopics(widget.subjectId, widget.languageId),
        showLoader: false,
      ),
    ]);
    if (!mounted) return;
    ...
    for (final topic in topics) {
      grouped.putIfAbsent(topic['chapterId'] as String, () => []).add(topic);
    }
    setState(() {
      _chapters = chapters;
  ...
  Widget _buildSubjectHeader() {
    ...
    return Column(
      ...
      children: [
        ...
        Row(
          children: [
            Expanded(child: _statPill('Chapters', '${_chapters.length}')),
            const SizedBox(width: 10),
            Expanded(
              child: _statPill('Questions', '${_totalQuestionsAllChapters()}'),
            ),
          ],
        ),
      ],
  ...
  Future<void> _openSubscription(String reason) async {
    ...
    final boardId = await AppSharedPreferencesData.getBoardId();
    final classId = await AppSharedPreferencesData.getClassId();
    if (!mounted) return;

    if (boardId.isEmpty || classId.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Please select your board and class first.'),
        ),
      );
      return;
    }
