...
import 'other_course_topics_screen.dart';
import 'other_course_subscription_screen.dart';

class OtherCourseChaptersScreen extends StatefulWidget {
  ...
  const OtherCourseChaptersScreen({
    super.key,
  ...
  @override
  State<OtherCourseChaptersScreen> createState() =>
      _OtherCourseChaptersScreenState();
}

class _OtherCourseChaptersScreenState extends State<OtherCourseChaptersScreen> {
  ...
  Future<void> _fetchData() async {
    ...
    final results = await Future.wait([
      APIService.getApiCaller(
        context: context,
        url: ApiUrls.getOtherCourseChapters(widget.otherCourseSubjectId),
        showLoader: false,
      ),
      APIService.getApiCaller(
        context: context,
        url: ApiUrls.getOtherCourseSubjectTopics(widget.otherCourseSubjectId),
        showLoader: false,
      ),
    ]);
    if (!mounted) return;
            ...
            data
                .map<Map<String, dynamic>>(
                  (e) => {
                    ...
                    'otherCourseChapterId': _idOf(e['otherCourseChapterId']),
                    'sortOrder': _toInt(e['sortOrder']),
                    'isLocked':
                        e['isLocked'] == true ||
    ...
    for (final topic in topics) {
      grouped
          .putIfAbsent(topic['otherCourseChapterId'] as String, () => [])
          .add(topic);
    }
    setState(() {
      _chapters = chapters;
