...
class _QuestionsScreenState extends State<QuestionsScreen> {
  ...
  Future<void> _selectOption(int optionIndex) async {
    ...
    final response = await APIService.postApiCaller(
      context: context,
      url: ApiUrls.submitAnswer(questionId),
  ...
  // whichever language the screen is currently showing (widget.languageId,
  // the same language its questions were fetched in). Once recorded, the
  // button for this question+language never comes back — see the
  // isNotEmpty check in build() below.
  ...
  Future<void> _goToNext() async {
    ...
    if (_currentIndex < _questions.length - 1) {
      ...
      if (!_topicReadOnly) {
        final response = await APIService.postApiCaller(
          context: context,
          url: ApiUrls.completeTopic(widget.topicId),
          body: {},
          showLoader: true,
        );
              ...
              child: SingleChildScrollView(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    // language (explanationVideoUrl, resolved server-side to
                    // widget.languageId), never shown to students at all. ──
                    if (_isTeacher) ...[
                      SizedBox(
                        width: double.infinity,
